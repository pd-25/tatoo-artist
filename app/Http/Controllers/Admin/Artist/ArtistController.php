...
class ArtistController extends Controller
{
    ...
    public function customers(Request $re)
    {
        ...
        if (Auth::guard('admins')->check()) {
            ...
            if (!empty($searchCustomer)) {
                if (is_numeric($searchCustomer)) {
                    $customerQuery->where('users.id', $searchCustomer);
                } else {
                    $customerQuery->where(function ($query) use ($searchCustomer) {
                        $query->where('users.username', 'LIKE', "%{$searchCustomer}%")
                            ->orWhere('users.name', 'LIKE', "%{$searchCustomer}%")
            ...
            // Always fetch full list for dropdown
            ...
            $customerQuery = User::where('type', 'Customer')
                ->where('created_by', $artistId)
                ->orderBy('id', 'DESC');

            if (!empty($searchCustomer)) {
                if (is_numeric($searchCustomer)) {
                    $customerQuery->where('id', $searchCustomer);
                } else {
                    $customerQuery->where(function ($query) use ($searchCustomer) {
