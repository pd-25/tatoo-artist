...
class DashboardController extends Controller
{
    ...
    public function userformsubmit(Request $request)
    {
        // Validate request inputs (uncomment and modify if needed)
        /*
        $this->validate($request, [
            'any_history_of_or_current_conditions_of' => 'required',
            'signature' => 'required|image|mimes:jpeg,png,jpg,gif',
        ...
        */
        ...
        $signature_path = $request->file('signature') 
            ? $request->file('signature')->store('tattoform', 'public')
            : '';
    
        $driving_licence_front_path = $request->file('driving_licence_front') 
            ? $request->file('driving_licence_front')->store('tattoform', 'public') 
            : '';
    
        $driving_licence_back_path = $request->file('driving_licence_back') 
            ? $request->file('driving_licence_back')->store('tattoform', 'public') 
            : '';
    
        // Create a new Tattoform record
        $tatto = new Tattoform();
        $tatto->user_id = $request['user_id'];
        $tatto->artist_id = $request['artist_id'];
        $tatto->general_good_health = $request['general_good_health'];
        ...
        // Fetch artist details from the users table
        $artist = User::where('id', $request['artist_id'])->first(); // Get the artist's name
    
        // Fetch the tattoo form data
        $tattodata = Tattoform::where('id', $tatto->id)->first();
    
        // Prepare data to pass to the view
        $data['artistdata'] = $artist; // Assign the artist object
        $data['tattodata'] = $tattodata; // Assign the tattoo form data
        ...
        // Send email to user
        $toemail = $artist->email; // Artist email
        Mail::send('admin.email.view-tatto', ['fullpath' => $path], function ($message) use ($toemail) {
            $message->to($toemail);
            $message->subject('User Tattoo Submitted Data.');
        });
